...
import { DollarSign, Users, ShoppingBag, TrendingUp, Printer, Calendar as CalendarIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const fetchStats = async (date) => {
        try {
            // Fix: Use local date string to avoid timezone issues (toISOString uses UTC)
    ...
    const handleDownloadReport = () => {
        const reportData = {
            ...
            recentTransactions: stats.recentBills.map(b => `ID: ${b.id}, Amount: ${b.total}`).join('; ')
        };
    ...
    const handleDownloadReceipt = (visit) => {
        ...
        receiptWindow.document.write(`
            <html>
            <head>
                <title>Receipt - Salon City</title>
                <style>
                                ...
                                stats.recentBills.map(bill => (
                                    <tr key={bill.id} onClick={() => handleBillClick(bill.visitId)} style={{ cursor: 'pointer', transition: 'background 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                                        ...
                                        <td>
                                            ...
                                            <div style={{ fontSize: '0.75rem', color: 'var(--muted-foreground)' }}>{bill.contactNo}</div>
                                        </td>
